...
class Book(db.Model):
    ...
    book_title = db.Column(db.String(20))
    book_length = db.Column(db.String(200))
    book_type = db.Column(db.String(50))
    user_token = db.Column(db.String, db.ForeignKey('user.token'), nullable = False)

    def __init__(self,ISBN,author,book_title,book_length,book_type,user_token, id = ''):
        self.id = self.set_id()
        self.ISBN = ISBN
        self.author = author
...
class BookSchema(ma.Schema):
    class Meta:
        fields = ['id', 'ISBN','author','book_title', 'book_length', 'book_type']
